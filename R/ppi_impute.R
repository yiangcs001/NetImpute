...
#'
#' @return The imputated data use interactions between genes in PPI network.
#' @export
#'
#' @examples
ppi_impute <- function(data, fcm_cluster_num, filter_thre, pca_dim, ppi,
                       p_low_cut, p_high_cut, lambda1, lambda2){
  print("Impute through PPI ...")
  # Filter data
  gene_fileter <- rowSums(data > 0) > (ncol(data) * filter_thre)
  filter_data <- data[gene_fileter,]
  # Normal the PPI network and data
  ppi_net <- ppi[(ppi[, 1] %in% rownames(filter_data) & ppi[, 2] %in% rownames(filter_data)), ]
  ppi_graph <- igraph::graph_from_data_frame(ppi_net, directed = FALSE)
  ppi_data <- filter_data[(rownames(filter_data) %in% ppi_net[, 1] | rownames(filter_data) %in% ppi_net[, 2]), ]
  # Determin the PCA dimension number
  dist_res <- as.matrix(1 - cor(ppi_data, method = "pearson"))
    ...
    if(length(cell_index) > 5){
      ...
      for(row_index in 1:nrow(temp)){
        ...
        if(sum(d_flag) > 0){
          ...
          temp_nb_gene <- igraph::neighbors(ppi_graph, g_name)
          nb_gene <- setdiff(intersect(names(unique(temp_nb_gene)), rownames(temp)), g_name )
          if(length(nb_gene) > 1){
            xx <- as.matrix(t(temp[nb_gene, !d_flag]))
            yy <- as.matrix(temp[row_index, !d_flag])
            ximpute <- as.matrix(t(temp[nb_gene, d_flag]))
            ...
            impute_data[g_name,][match(names(d_flag)[which(d_flag)], colnames(impute_data))] <- yimpute
          }
        }
      }
  ...
  if(length(unidentify_data) == 0){
    ...
    for(temp_index in 1:length(unidentify_data)){
      ...
      ppi_impute_data[, cell_index] <- apply(temp_data, 1, max)
    }
  }
  ppi_impute_data <- ppi_data
  return(list(impute_data = ppi_impute_data, pca_dim = pca_dim, outliers = outliers))
}
